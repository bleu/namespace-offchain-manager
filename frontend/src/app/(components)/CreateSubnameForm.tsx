...
export const CreateSubnameForm = ({
  ...
  const filteredEnsNames = ensNames?.filter(ens => 
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <div className="grid grid-cols-1 gap-4">
                  ...
                  {filteredEnsNames.length > 0 ? (
                    filteredEnsNames.map((ens) => (
                      <button
                        ...
                        onClick={() => {
                          setFormData(prev => ({ ...prev, parentName: ens.name ?? '' }));
                          setIsOpen(false);
                          setSearchTerm("");
                        }}
                      >
        ...
        <div className="space-y-2">
          <label htmlFor="subname" className="text-sm font-medium">Subname</label>
          <Input
            id="subname"
      ...
      <div className="flex justify-end gap-2">
        ...
        <Button type="submit" disabled={isSubmitting}>
          {isSubmitting ? 'Saving...' : subname ? 'Update Subname' : 'Create Subname'}
        </Button>
      </div>
    </form>
  );
